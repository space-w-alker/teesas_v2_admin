...
import {
  ...
  deleteBankAsync,
  getBankListResponse,
} from "../../apis/slices/bankSlice";
import { config } from "../../apis/client/config";
import {
  getWithdrawRequest,
  getWithdrawRequestAsync,
} from "../../apis/slices/teacherSlice";

const WithdrawRequest = ({ isOpen }) => {
    ...
    getWithdrawRequestAsync({
      ...
      callbackFn: (response) => {
        ...
        if (response?.data?.status === 200) {
          setBanks(response.data.data.requests || []);
          setTotalBanks(response.data.data.totalRequests || 0);
        } else {
          setBanks([]);
          setTotalBanks(0);
          console.error("Error loading Requests:", response?.data?.message);
        }
      },
    });
  };
  ...
  return (
    ...
    >
      <Headers value1="Home" value2="Withdraw requests" />

      <div className="p-6 border-b border-gray-100">
        <div className="flex justify-between items-center">
          <Headcomponent value="Withdraw Requests" showSearch={false} />
        </div>
      </div>

      <div className="mt-3">
        <UserCard
          label="Total Withdraw Requests"
          height="h-[140px]"
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Custombutton
                    ...
                    textcolor="text-[#27AE60]"
                    onClick={() =>
                      navigate(`/payments/withdraw-details`, {
                        state: { data: bank },
                      })
                    }
                  />
