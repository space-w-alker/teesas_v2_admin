...
const WebinarRegistrationList = ({ onRefresh }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { response, isLoading } = useSelector(getWebinarRegistrationsResponse);
    ...
    getWebinarRegistrationsAsync({
      ...
      callbackFn: (res) => {
        ...
        if (res?.data) {
          ...
          setPageData({
            currentPage: res.data.data?.paging?.currentPage || res.data.paging?.currentPage || 1,
            totalPage: res.data.data?.paging?.totalPage || res.data.paging?.totalPage || 1
          });
        }
      },
      data: {
              ...
              <li key={registration?.id || index}>
                <div className="flex justify-between gap-4 items-center">
                  ...
                  <div className="flex gap-2">
                    <Custombutton
                      value="View"
                      onClick={() => navigate(`/webinar-registrations/details/${registration?.id}`)}
...
export default WebinarRegistrationList;
