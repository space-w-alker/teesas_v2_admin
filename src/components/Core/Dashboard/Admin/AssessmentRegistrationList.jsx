...
const AssessmentRegistrationList = ({ onRefresh }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { response, isLoading } = useSelector(getAssessmentRegistrationsResponse);
              ...
              <li key={registration?.id}>
                <div className="flex justify-between gap-4 items-center">
                  <div className="px-[18px] py-[10px] mt-5 flex items-center gap-[10px] pr-[15px] flex-1">
                    <div className="flex-1">
                      ...
                      <div className="px-[18px] mt-1">
                        <p className="text-[12px] text-gray-500">Parent: {registration?.parentName || 'N/A'}</p>
                        <p className="text-[12px] text-gray-500">{registration?.parentEmail || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Custombutton
                      value="View"
                      onClick={() => navigate(`/assessment-registrations/details/${registration?.id}`)}
...
export default AssessmentRegistrationList;
