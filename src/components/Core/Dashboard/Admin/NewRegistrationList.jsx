...
import { getSignUpRegistrationsAsync, getSignUpRegistrations, getSignUpRegistrationsResponse } from "../../../../apis/slices/signUpRegistrationSlice";
import { FaArrowLeft, FaArrowRight } from 'react-icons/fa';

const NewRegistrationList = ({ onRefresh }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { response, isLoading } = useSelector(getSignUpRegistrationsResponse);
  ...
  const fetchRegistrations = () => {
    ...
    getSignUpRegistrationsAsync({
      dispatch,
      callbackFn: (res) => {
        setLoading(false);
        if (res?.data) {
          setRegistrationData(res.data.data?.data || res.data.data || []);
          setPageData({
              ...
              <li key={registration?.id || index}>
                <div className="flex justify-between gap-4 items-center">
                  <div className="px-[18px] py-[10px] mt-5 flex items-center gap-[10px] pr-[15px] flex-1">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 px-[18px]">
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-[14px] leading-[16px] text-[#171717]">
                            {registration?.name || registration?.fullName || registration?.studentName || 'N/A'}
                          </p>
                          {registration?.status && (
                      ...
                      <div className="px-[18px] mt-1">
                        ...
                        <p className="text-[12px] text-gray-500">{registration?.phoneNumber || registration?.phone || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Custombutton
                      value="View"
                      onClick={() => navigate(`/new-registrations/details/${registration?.id}`)}
