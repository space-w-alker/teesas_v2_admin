...
const DemoRequestList = ({ onRefresh }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { response, isLoading } = useSelector(getDemoRequestsResponse);
  ...
  const fetchRequests = () => {
    ...
    getDemoRequestsAsync({
      ...
      callbackFn: (res) => {
        setLoading(false);
        if (res?.data) {
          setRequestData(res.data.data.data || []);
          setPageData({
              ...
              <li key={request?.id}>
                <div className="flex justify-between gap-4 items-center">
                  <div className="px-[18px] py-[10px] mt-5 flex items-center gap-[10px] pr-[15px] flex-1">
                    <div className="flex-1">
                      ...
                      <div className="px-[18px] mt-1">
                        <p className="text-[12px] text-gray-500">Requester: {request?.requesterName || 'N/A'}</p>
                        <p className="text-[12px] text-gray-500">{request?.email || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Custombutton
                      value="View"
                      onClick={() => navigate(`/demo-requests/details/${request?.id}`)}
