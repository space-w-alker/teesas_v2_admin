...
const ResellerForm = ({ isOpen }) => {
  ...
  const handlePageChange = (newPage) => {
    if (
      newPage < 1 ||
      (resellersState.pagination &&
        newPage > resellersState.pagination.total_pages)
    ) {
      return;
    }
  ...
  const renderResellerItems = () => {
    if (
      !resellersState.data ||
      !Array.isArray(resellersState.data) ||
      resellersState.data.length === 0
    ) {
      if (
        resellersState.statistics &&
        resellersState.statistics.total_resellers > 0
      ) {
        return (
    ...
    const isGroupedByDate =
      resellersState.data[0] &&
      resellersState.data[0].date &&
      resellersState.data[0].forms;
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-xl shadow-sm">
        ...
        <div className="p-6">{renderResellerItems()}</div>
        ...
        {resellersState && resellersState.pagination && (
          <div className="p-6 border-t border-gray-100 flex justify-between items-center">
            ...
            <Custombutton
              ...
              onClick={() => handlePageChange(currentPage + 1)}
              disabled={
                currentPage >= (resellersState.pagination.total_pages || 1)
              }
            />
          </div>
        )}
