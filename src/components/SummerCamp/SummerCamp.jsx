...
const ParticipantItem = ({ name, status }) => {
  ...
  return (
    <div
      ...
      onClick={() => navigate('/summer-camp-details', {
        state: {
          name: name,
          status: status
        }
...
const SummerCamp = ({ isOpen }) => {
  const [showModal, setShowModal] = useState(false);
  const navigate = useNavigate();

  const handleSingleParticipant = () => {
    navigate('/add-single-summer-camp-participant');
    setShowModal(false);
  };

  const handleBulkParticipant = () => {
    navigate('/add-bulk-summer-camp-participant');
    setShowModal(false);
  };
  ...
  return (
    <div className={`py-[7rem] lg:px-[5rem] px-[10px] ${isOpen ? "xl:ml-[260px]" : ""} transition-all duration-300`}>
      <Headers value1="Home" value2="Summer Camp" />
