...
export const createCategoryAsync = (categoryData) => async (dispatch) => {
  try {
    dispatch(
      setCategoryCreate({ isLoading: true, success: false, error: null })
    );
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setCategoryCreate({ isLoading: false, success: true, error: null })
      );
...
export const deleteCategoryAsync = (categoryId) => async (dispatch) => {
  try {
    dispatch(
      setCategoryDelete({ isLoading: true, success: false, error: null })
    );
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setCategoryDelete({ isLoading: false, success: true, error: null })
      );
      dispatch(getCategoriesAsync());
      return true;
    } else {
...
export const updateCategoryAsync =
  (categoryId, categoryData) => async (dispatch) => {
    try {
      dispatch(
        setCategoryUpdate({ isLoading: true, success: false, error: null })
      );
      ...
      if (result?.data?.status === 200) {
        dispatch(
          setCategoryUpdate({ isLoading: false, success: true, error: null })
        );
        dispatch(getCategoriesAsync());
        return true;
      } else {
...
export const addClassAsync = (categoryId, className) => async (dispatch) => {
  try {
    ...
    dispatch(
      setAddClass({ isLoading: false, success: false, error: error.message })
    );
    return false;
  }
};
...
export const deleteClassAsync = (classId) => async (dispatch) => {
  try {
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setDeleteClass({ isLoading: false, success: true, error: null })
      );
      return true;
    }
    ...
    dispatch(
      setDeleteClass({ isLoading: false, success: false, error: error.message })
    );
    return false;
  }
};
...
export const createSubjectAsync = (classId, formData) => async (dispatch) => {
  try {
    dispatch(
      setCreateSubject({ isLoading: true, success: false, error: null })
    );
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setCreateSubject({ isLoading: false, success: true, error: null })
      );
...
export const updateSubjectAsync =
  (classId, subjectId, formData) => async (dispatch) => {
    try {
      dispatch(
        setUpdateSubject({ isLoading: true, success: false, error: null })
      );
      ...
      if (result?.data?.status === 200) {
        dispatch(
          setUpdateSubject({ isLoading: false, success: true, error: null })
        );
        dispatch(getSubjectDetailsAsync(classId)); // Refresh the subjects list
        return true;
      }
...
export const deleteSubjectAsync = (classId, subjectId) => async (dispatch) => {
  try {
    dispatch(
      setDeleteSubject({ isLoading: true, success: false, error: null })
    );
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setDeleteSubject({ isLoading: false, success: true, error: null })
      );
...
export const deleteChapterAsync = (chapterId) => async (dispatch) => {
  try {
    dispatch(
      setDeleteChapter({ isLoading: true, success: false, error: null })
    );
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setDeleteChapter({ isLoading: false, success: true, error: null })
      );
      return true;
    }
...
export const updateChapterAsync =
  (chapterId, chapterData) => async (dispatch) => {
    try {
      dispatch(
        setCreateChapter({ isLoading: true, success: false, error: null })
      );
      const URL = `${BASEURL}admin/chapter/${chapterId}`;
      ...
      if (result?.data?.status === 200) {
        dispatch(
          setCreateChapter({ isLoading: false, success: true, error: null })
        );
        return true;
      }
...
export const createChapterAsync =
  (subjectId, chapterData) => async (dispatch) => {
    try {
      dispatch(
        setCreateChapter({ isLoading: true, success: false, error: null })
      );
...
export const createTopicAsync = (chapterId, topicData) => async (dispatch) => {
  try {
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setCreateTopic({ isLoading: false, success: true, error: null })
      );
    ...
    dispatch(
      setCreateTopic({ isLoading: false, success: false, error: error.message })
    );
    return null;
  }
};
...
export const deleteTopicAsync = (topicId) => async (dispatch) => {
  try {
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setDeleteTopic({ isLoading: false, success: true, error: null })
      );
      return true;
    }
    ...
    dispatch(
      setDeleteTopic({ isLoading: false, success: false, error: error.message })
    );
    return false;
  }
};
export const updateTopicAsync = (topicId, topicData) => async (dispatch) => {
  try {
    ...
    if (result?.data?.status === 200) {
      dispatch(
        setUpdateTopic({ isLoading: false, success: true, error: null })
      );
      return true;
    }
    ...
    dispatch(
      setUpdateTopic({ isLoading: false, success: false, error: error.message })
    );
    return false;
  }
};

export const uploadTopicMediaAsync =
  (lessonId, formData) => async (dispatch) => {
    ...
    try {
      ...
      if (result?.data?.status === 200) {
        dispatch(
          setTopicMedia({ isLoading: false, success: true, error: null })
        );
        return true;
      }
...
export const getTopicDetailAsync = (lessonId) => async (dispatch) => {
  try {
    ...
    const URL = `${BASEURL}admin/lesson/${lessonId}/details`;
...
export const getTopicWithChapterAsync = (lessonId) => async (dispatch) => {
  try {
    ...
    const URL = `${BASEURL}admin/lesson/${lessonId}/with-chapter`;
    ...
    if (result?.data?.status === 200) {
      ...
      throw new Error(result?.data?.message || "Failed to fetch topic details");
    }
  } catch (error) {
...
export const deleteTopicMediaAsync =
  (lessonId, mediaId) => async (dispatch) => {
    try {
      dispatch(
        setDeleteTopicMedia({ isLoading: true, success: false, error: null })
      );
      ...
      if (result?.data?.status === 200) {
        dispatch(
          setDeleteTopicMedia({ isLoading: false, success: true, error: null })
        );
        return true;
      }
...
export const getUniversitiesAsync = () => async (dispatch) => {
  try {
    ...
    dispatch(
      setUniversities({ isLoading: false, data: null, error: error.message })
    );
  }
};
...
export const getCountriesAsync = () => async (dispatch) => {
  try {
    ...
    dispatch(
      setCountries({ isLoading: false, data: null, error: error.message })
    );
  }
};
...
export const selectDeleteTopicMedia = (state) =>
