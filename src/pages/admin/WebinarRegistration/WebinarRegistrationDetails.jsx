...
import WebinarRegistrationComponent from '../../../components/Core/Dashboard/Admin/WebinarRegistrationComponent'
import { useDispatch } from 'react-redux'
import { FaChevronLeft } from "react-icons/fa"
import { useParams, useNavigate } from 'react-router-dom'
import { TailSpin } from "react-loader-spinner"
import { getWebinarRegistrationByIdAsync, getWebinarRegistrationById } from '../../../apis/slices/webinarRegistrationSlice'

const WebinarRegistrationDetails = ({ isOpen }) => {
  const { id } = useParams()
  ...
  const fetchRegistrationDetails = () => {
    ...
    getWebinarRegistrationByIdAsync({
      dispatch,
      registrationId: id,
      callbackFn: (res) => {
        setLoading(false)
  ...
  const handleGoBack = () => {
    navigate('/webinar-registrations')
  }
    ...
    <div className={`py-[7rem] lg:px-[5rem] px-[10px] ${isOpen ? "xl:ml-[260px]" : ""}`}>
      ...
      <div className='flex justify-start items-center lg:gap-3 cursor-pointer' onClick={handleGoBack}>
        ...
        <div>
          <div className='font-normal text-[14px] lg:text-[16px] leading-[20px] text-[#B6B6B6]'>
            Home / Webinar Registration /<span className='text-black font-medium'>Registration Details</span>
          </div>
        </div>
      </div>
      ...
      {error ? (
        ...
        <>
              ...
              <div className="flex items-start gap-4">
                <div className="w-[50px] h-[50px] rounded-full overflow-hidden bg-green-100 flex items-center justify-center flex-shrink-0">
                  <span className="text-green-600 font-bold text-xl">
                    {registrationData?.name?.charAt(0)?.toUpperCase() || "W"}
                  </span>
                </div>
          ...
          <WebinarRegistrationComponent registrationData={registrationData} />
        </>
      )}
    </div>
  )
...
export default WebinarRegistrationDetails
