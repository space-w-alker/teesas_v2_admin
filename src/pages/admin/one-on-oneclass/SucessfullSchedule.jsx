        ...
        <div className=" rounded-[16px] bg-[#FFFFFF] flex flex-col p-4 gap-3 lg:gap-6 justify-center items-center lg:w-[700px] ">
          ...
          <div className=" rounded-lg py-[10px] px-[20px] bg-[#FFF9ED] flex justify-between w-[280px] lg:w-[500px] h-[78px]">
            <div>
              ...
              >
                {data?.data?.zoom_meeting_url}
              </a>
            </div>
            <div className="mt-2">
              <p onClick={()=>{
                navigator.clipboard.writeText(data?.data?.zoom_meeting_url);
                toast.success("Your Live Class Link Copied")
              }} className=" text-[12px] text-center lg:text-[16px] leading-[19px] py-[5px] rounded-[40px] px-[17px] lg:pt-[5px] lg:pb-[9px] cursor-pointer  border border-[#000000] w-[80px]">
              Copy
                </p>
            </div>
          </div>
          <div className="flex  gap-2 lg:gap-4" >
            <button className=" rounded-lg border border-[#F2994A] w-[120px] text-[14px] text-[#F2994A] leading-[20px] text-center lg:w-[244px] h-[40px]" onClick={()=>{Navigate('/one-on-oneclassmanagement')}}>Back to Home</button>
            <button onClick={()=>{
              navigator.clipboard.writeText(data?.data?.zoom_meeting_url);
              toast.success("Your Live Class Link Copied")
            }}  className="rounded-lg border border-[#F2994A] w-[137px] text-[14px] text-[#F2994A] leading-[20px] text-center lg:w-[244px] h-[40px]">Send Class Invitation</button>
          </div>
        </div>
      </Popup>
      <ToastContainer
