...
import {getTeacherStatsAsync} from "../../../apis/slices/teacherSlice"
...
const TeacherList = ({isOpen}) => {
  ...
  useEffect(() => {
    setLoading(true)
    getTeacherStatsAsync({
      dispatch: dispatch,
        data:{},
      token: token,
      callbackFn: (res) => {
        if (res?.data?.status === 200) {
          setAdminData(res?.data?.data?.statistics);
          setLoading(false)
        }
        else{
  ...
  return (
    ...
    <div className=" mt-3 ">
      <UserCard
        ...
        value={adminData?.total_teachers}
        imgbg={live}
        imglogo={liveimage}
      />
    </div>
    <div className=" flex  justify-end mt-4 ">
      <button className="text-[14px] leading-[20px] text-center font-bold  w-[181px] h-[40px] rounded-lg py-[7px] px-[12px] bg-[#F2994A] text-white" onClick={()=>Navigate('/Teacher/AddTeacher')}>
       + Add Teacher   
      </button>
    </div>
    <TeacherLists/>
